...
public class FTuple_DataModel2EObjectDataModel7 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataModel relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_DataModel() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//DataModel" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature1 = objectClass1.getEStructuralFeature("imports");
				
		List<FTuple> linkFTuples1 = source.getInstance().getTuples("get_imports_DataModel_to_DataModel_imports");
		List<FTuple> dataFTuples1 = source.getInstance().getTuples("get_DataConcept"); 
		...
		EStructuralFeature feature3 = objectClass3.getEStructuralFeature("owns");
				
		List<FTuple> linkFTuples3 = source.getInstance().getTuples("get_owns_DataModel_to_DataModel_owns");
		...
		EStructuralFeature feature4 = objectClass4.getEStructuralFeature("notesThat");
				
		List<FTuple> linkFTuples4 = source.getInstance().getTuples("get_notesThat_CatalogueCore_to_CatalogueCore_notesThat");
		List<FTuple> dataFTuples4 = source.getInstance().getTuples("get_DataAnnotation"); 
