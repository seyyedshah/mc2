...
public class FTuple_Enumeration2EObjectEnumeration21 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_Enumeration relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_Enumeration() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//Enumeration" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature4 = objectClass4.getEStructuralFeature("has");
				
		List<FTuple> linkFTuples4 = source.getInstance().getTuples("get_has_Enumeration_to_Enumeration_has");
		List<FTuple> dataFTuples4 = source.getInstance().getTuples("get_EnumLiteral"); 
