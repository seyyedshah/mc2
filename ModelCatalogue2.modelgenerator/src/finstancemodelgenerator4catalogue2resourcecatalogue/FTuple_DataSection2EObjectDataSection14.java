...
public class FTuple_DataSection2EObjectDataSection14 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataSection relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_DataSection() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//DataSection" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature3 = objectClass3.getEStructuralFeature("groups");
				
		List<FTuple> linkFTuples3 = source.getInstance().getTuples("get_groups_DataSection_to_DataSection_groups");
		List<FTuple> dataFTuples3 = source.getInstance().getTuples("get_DataClass"); 
