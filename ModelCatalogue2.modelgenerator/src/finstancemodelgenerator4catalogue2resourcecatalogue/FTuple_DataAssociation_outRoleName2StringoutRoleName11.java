...
public class FTuple_DataAssociation_outRoleName2StringoutRoleName11 implements Rule<FTuple,String>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataAssociation_outRoleName relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_DataAssociation_outRoleName() );
	}
