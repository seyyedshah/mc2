...
public class FTuple_DataAnnotation2EObjectDataAnnotation2 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataAnnotation relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_DataAnnotation() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//DataAnnotation" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature2 = objectClass2.getEStructuralFeature("content");
				
		List<FTuple> linkFTuples2 = source.getInstance().getTuples("get_content_DataAnnotation_to_DataAnnotation_content");
		List<FTuple> dataFTuples2 = source.getInstance().getTuples("get_DataAnnotation_content"); 
		...
		EStructuralFeature feature3 = objectClass3.getEStructuralFeature("notesThat");
				
		List<FTuple> linkFTuples3 = source.getInstance().getTuples("get_notesThat_CatalogueCore_to_CatalogueCore_notesThat");
		List<FTuple> dataFTuples3 = source.getInstance().getTuples("get_DataAnnotation"); 
		...
		EStructuralFeature feature4 = objectClass4.getEStructuralFeature("languageURI");
				
		List<FTuple> linkFTuples4 = source.getInstance().getTuples("get_languageURI_DataAnnotation_to_DataAnnotation_languageURI");
		List<FTuple> dataFTuples4 = source.getInstance().getTuples("get_DataAnnotation_languageURI"); 
		...
		EStructuralFeature feature5 = objectClass5.getEStructuralFeature("type");
				
		List<FTuple> linkFTuples5 = source.getInstance().getTuples("get_type_DataAnnotation_to_DataAnnotation_type");
		List<FTuple> dataFTuples5 = source.getInstance().getTuples("get_DataAnnotation_type"); 
