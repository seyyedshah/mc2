...
public class FTuple_DataClass2EObjectDataClass15 implements Rule<FTuple,EObject>{

	public boolean check(FTuple source){
		//check that there are no sub rules that convert the element
		//check the FTuple is a tuple of the get_DataClass relation 
		return source.instanceOf( ((ModelGenerator4Catalogue)source.getInstance().getMetaModel()).get_DataClass() );
	}
	...
	public EObject build(FTuple source, Transformer t){
		...
		EClass metaClass = (EClass) metaModel.getEObject( "//DataClass" );
	...
	public void setProperties(EObject target, FTuple source, Transformer t){
		...
		EStructuralFeature feature1 = objectClass1.getEStructuralFeature("classifies");
				
		List<FTuple> linkFTuples1 = source.getInstance().getTuples("get_classifies_DataClass_to_DataClass_classifies");
		List<FTuple> dataFTuples1 = source.getInstance().getTuples("get_DataElement"); 
		...
		EStructuralFeature feature2 = objectClass2.getEStructuralFeature("name");
				
		List<FTuple> linkFTuples2 = source.getInstance().getTuples("get_name_CatalogueCore_to_CatalogueCore_name");
		List<FTuple> dataFTuples2 = source.getInstance().getTuples("get_CatalogueCore_name"); 
		...
		EStructuralFeature feature4 = objectClass4.getEStructuralFeature("clones");
				
		List<FTuple> linkFTuples4 = source.getInstance().getTuples("get_clones_DataConcept_to_DataConcept_clones");
